...
def readExamples(path):
    ...
    with open(path) as csv_file:
        ...
        for row in csv_reader:
            ...
            if (line_count != 0):
                #print(type(row[3]))
                examples.append((row[1].strip(), int(float(row[0])))) #append review and number
            line_count += 1
            #print(line_count)
            # if line_count == 0:
    ...
    # with open("shortishreviews.json") as f:
    ...
    #     print(x)
    #     #y, x = line.split(' ', 1) 
    #     examples.append((x.strip(), y)) #append review and number
